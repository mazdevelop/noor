...
// دسته‌بندی‌ها با توضیحات کامل‌تر
const categories = [
  {
    id: 1,
...
const ProductsPage = () => {
  const [currentCategory, setCurrentCategory] = useState(categories[0]); // دسته‌بندی پیش‌فرض
  const router = useRouter();

  const filteredProducts = allProducts.filter(product => product.categoryId === currentCategory.id);

  const handleProductClick = (product: Product) => {
    const category = categories.find(cat => cat.id === product.categoryId);

    // تبدیل query params به string با استفاده از URLSearchParams
    ...
    // انتقال به صفحه محصول با استفاده از query params به صورت string
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {categories.map((category) => (
          <button
            ...
            className={`flex items-center justify-center p-2 rounded-lg hover:bg-gray-50 transition-all duration-300 ${currentCategory.id === category.id ? 'bg-primary-100' : ''}`}
          >
            {category.icon && (
              <Image src={category.icon} alt={category.name} width={32} height={32} className="object-fill w-10 h-10 mx-1" />
            )}
            <span className="text-xs text-gray-700 text-center block w-full">{category.title}</span>
          </button>
        ))}
      </div>
      ...
      {/* نمایش محصولات مرتبط با دسته‌بندی انتخاب‌شده */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {filteredProducts.map((product) => (
          <div key={product.id} className="bg-white shadow-md rounded-lg p-4" onClick={() => handleProductClick(product)}>
              <div>
